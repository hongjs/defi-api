...
const { getGodfatherStorageAddress: contractAddress } = require('./addressHelpers')
const storageAbi = require('../configs/abi/godfatherStorage.json')
...
const getInfo = async (id) => {
  ...
  const info = {
    ...
    rarity: new BigNumber(_info._rarity._hex).toNumber(),
...
const getInfos = async (ids) => {
  ...
  const infos = results.map((result) => {
    return {
      ...
      rarity: new BigNumber(result._rarity._hex).toNumber(),
